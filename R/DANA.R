...
#'   \item Remove Unwanted Variation (RUV)
#' }
#'
...
compute.mscr <- function(rawCor, normCor) {
  ...
  mscr <- (varZero.rawCor - varZero.normCor) / varZero.rawCor
  return(list(mscr=mscr,
              rawCor=rawCor,
              normCor=normCor))
}
...
compare.models <- function(rawModel,
  ...
  comparison.neg <- compute.mscr(rawCor=rawModel$neg$corr, normCor=normModel$neg$corr)
...
DANA <- function(data.RC,
  ...
  metrics <- data.frame(t(sapply(comparison, function(x) c(x$cc, x$mscr))))
  colnames(metrics) <- c("cc", "mscr")

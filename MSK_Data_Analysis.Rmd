---
title: "Normalization Performance Analysis for the Paired MSK Data Sets"
...
if(settings$all) {
  ...
  settings$compare.corr.methods=TRUE
  settings$compare.two.methods=TRUE
...
if(settings$compare.corr.methods) {
  ...
  # # Display mean correlation reduction in negative controls
  # compare.test.mcr <- cbind(DANA.test.mb.bic.res$metrics$mcr,
  #                           DANA.test.mb.aic.res$metrics$mcr,
  #                           DANA.test.mb.cv.res$metrics$mcr,
  #                           DANA.test.mb.av.res$metrics$mcr,
  #                           DANA.test.glasso.ric.res$metrics$mcr,
  #                           DANA.test.glasso.stars.res$metrics$mcr,
  #                           DANA.test.huge.mb.ric.res$metrics$mcr,
  #                           DANA.test.huge.mb.stars.res$metrics$mcr,
  #                           DANA.test.fastggm.res$metrics$mcr,
  #                           DANA.test.pearson.res$metrics$mcr,
  #                           DANA.test.spearman.res$metrics$mcr)
  # rownames(compare.test.mcr) <- rownames(DANA.test.mb.bic.res$metrics)
  # colnames(compare.test.mcr) <- c("mb.bic", "mb.aic", "mb.cv", "mb.av", "glasso.ric", "glasso.stars", "huge.mb.ric", "huge.mb.stars", "fastggm", "pearson", "spearman")
  # stargazer(compare.test.mcr, type="text", summary=FALSE, digits=5, 
  #             title="Test data - mcr-", align=TRUE)
  # corrplot.mixed(cor(compare.test.mcr),lower="circle",upper="number")
  ...
  # huge.mb (ric)
  p.test.huge.mb.ric.stats <- plot.DANA.metrics(DANA.test.huge.mb.ric.res$metrics, label.repel=TRUE) + ggtitle("huge.mb (ric)")
  print(p.test.huge.mb.ric.stats)
  # huge.mb (stars)
  p.test.huge.mb.stars.stats <- plot.DANA.metrics(DANA.test.huge.mb.stars.res$metrics, label.repel=TRUE) + ggtitle("huge.mb (stars)")
...
In this section, we compare two methods for estimating the co-expression of miRNAs to each other. The methods can be chosen individually to positive and negative controls and include all available marginal and partial correaltion methods.
...
if(settings$compare.cutoffs) {
  ...
  cutoff.conf2 <- list(
    t.zero = 1.5,  # zero-expressed in [0, t.zero)
    t.poor = 5,  # poorly-expressed in [t.zero, t.poor]
...
if(settings$generate.paper.figs) {
  ...
  neg.corr <- data.frame(
    control=factor(c(rep("Benchmark", length(nnz.corr.bench.neg)), 
                     rep("Test", length(nnz.corr.test.neg)),
                     rep("Test (TMM)", length(nnz.corr.test.norm1.neg)))),
  ...
  neg.corr <- data.frame(
    control=factor(c(rep("Benchmark", length(nnz.corr.bench.neg)), 
                     rep("Test", length(nnz.corr.test.neg)),
                     rep("Test (TMM)", length(nnz.corr.test.norm1.neg)),
                     rep("Test (RUVs)", length(nnz.corr.test.norm2.neg)))),
  ...
  neg.corr <- data.frame(
    control=factor(c(rep("Benchmark", length(nnz.corr.bench.neg)), 
                     rep("Benchmark (TMM)", length(nnz.corr.bench.norm1.neg)),
                     rep("Test", length(nnz.corr.test.neg)),
                     rep("Test (TMM)", length(nnz.corr.test.norm1.neg)))),
  ...
  if(settings$export.figures) {
    # ggsave(paste0(settings$paper.fig.path, "MSK_CorrDensityNeg_Hist.pdf"), p.density.neg.nnz.hist, width = 5, height=4, device="pdf")
    ggsave(paste0(settings$paper.fig.path, "MSK_CorrDensityNeg_Freq.pdf"), p.density.neg.nnz.freq, width = 5, height=4, device="pdf")
    ggsave(paste0(settings$paper.fig.path, "MSK_CorrDensityNeg_Freq_WithBench.pdf"), p.density.neg.nnz.freq.four, width = 5, height=4, device="pdf")
  }
}
```
...
if(settings$generate.paper.figs) {
  ...
  control <- c(rep("Benchmark", length(nnz.corr.bench.neg)), 
                     rep("Test", length(nnz.corr.test.neg)))
  for(i in 1:length(nnz.corr.test.norm.neg)) {
    control <- c(control, rep(paste0("Test (", names(nnz.corr.test.norm.neg)[i],")"), length(nnz.corr.test.norm.neg[[i]])))
  }
  ## direct comparison of negative controls benchmark vs test vs test.norm1
  neg.corr <- data.frame(
  ...
  if(settings$export.figures) {
    # ggsave(paste0(settings$paper.fig.path, "MSK_CorrDensityNeg_Hist.pdf"), p.density.neg.nnz.hist, width = 5, height=4, device="pdf")
    ggsave(paste0(settings$paper.fig.path, "MSK_CorrDensityNeg_Freq_All.pdf"), p.corr.frequency.neg.controls.all, width = 8, height=5, device="pdf")
  }
}
```
...
if(settings$generate.paper.figs) {
  ...
  p.scatter.test.TMM <- plot.corr.scatter(
    ...
    ylab="TMM", 
    threshold=0,
    ccc=DANA.test$metrics["test.TMM", "cc"])
  ...
  p.scatter.test.UQ <- plot.corr.scatter(
    ...
    ylab="UQ", 
    threshold=0,
    ccc=DANA.test$metrics["test.UQ", "cc"])
  ...
  if(settings$export.figures) {
    ...
    # ggsave(paste0(settings$paper.fig.path, "MSK_CorrNeg.eps"), p.corr.neg, width = 8, height=3.5, device="eps")
  }
}
```
...
if(settings$generate.paper.figs) { 
  ...
  if(settings$export.figures) {
    ...
    ggsave(paste0(settings$paper.fig.path, "MSK_MetricsBench.pdf"), p.metrics.bench, width=4.5, height=3.5, device="pdf")
  }
...
if(settings$generate.paper.figs && settings$compare.cutoffs) { 
  ...
  p.cutoffs <-
    plot_grid(p.RC.conf1,
              p.RC.conf2 + labs(x="Mean", y=element_blank()),
              p.RC.conf3 + labs(x="Mean", y=element_blank()),
              p.meanvar.conf1,
              p.meanvar.conf2 + labs(x="Mean", y=element_blank()),
              p.meanvar.conf3 + labs(x="Mean", y=element_blank()),
              p.result.DANA.1,
              p.result.DANA.2 + labs(y=element_blank()),
...
if(settings$generate.paper.figs && settings$compare.corr.methods) { 
  methods.name <- c("mb (bic)", "mb (aic)", "mb (cv)","mb (av)", "glasso (ric)", "glasso (stars)", "huge.mb (ric)", "huge.mb (stars)", "fastggm", "pearson", "spearman")
  ...
  # huge.mb (ric)
  p.test.huge.mb.ric.stats <- plot.DANA.metrics(DANA.test.huge.mb.ric.res$metrics, label.repel=TRUE, label.size = 2.5) + ggtitle("huge.mb (ric)") + theme(plot.title = element_text(size=12))
  # huge.mb (stars)
  p.test.huge.mb.stars.stats <- plot.DANA.metrics(DANA.test.huge.mb.stars.res$metrics, label.repel=TRUE, label.size = 2.5) + ggtitle("huge.mb (stars)") + theme(plot.title = element_text(size=12))
  # FastGGM
  ...
  p.partialCorr.compare.metrics <-
    plot_grid(p.test.mb.bic.stats + labs(x="mcr-", y="cc+"),
              p.test.mb.aic.stats + labs(x="mcr-", y=element_blank()),
              p.test.mb.cv.stats + labs(x="mcr-", y=element_blank()),
              p.test.mb.av.stats + labs(x="mcr-", y="cc+"),
              p.test.glasso.ric.stats + labs(x="mcr-", y=element_blank()),
              p.test.glasso.stars.stats + labs(x="mcr-", y=element_blank()),
              p.test.huge.mb.ric.stats + labs(x="mcr-", y="cc+"),
              p.test.huge.mb.stars.stats + labs(x="mcr-", y=element_blank()),
              p.test.fastggm.stats + labs(x="mcr-", y=element_blank()),
              p.test.pearson.stats + labs(x="mcr-", y="cc+"),
              p.test.spearman.stats + labs(x="mcr-", y=element_blank()),
              ncol=3,
              align="hv")
  ...
  p.corr.huge.mb.ric <- ggplot.corr(DANA.test.huge.mb.ric.res$data.model$pos$corr, clusters, title="huge.mb (ric)")
  p.corr.huge.mb.stars <- ggplot.corr(DANA.test.huge.mb.stars.res$data.model$pos$corr, clusters, title="huge.mb (stars)")
